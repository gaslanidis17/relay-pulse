...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN sla.is_late THEN ap.id END) AS late_orders_official,
    ...
    COUNT(DISTINCT CASE WHEN COALESCE(fcd.IS_HEAVY_DELIVERY, FALSE) THEN ap.id END) AS total_heavy,
    COUNT(DISTINCT CASE WHEN COALESCE(fcd.IS_LARGE_DELIVERY, FALSE) THEN ap.id END) AS total_large,
    COUNT(DISTINCT CASE WHEN COALESCE(fcd.IS_HEAVY_DELIVERY, FALSE) OR COALESCE(fcd.IS_LARGE_DELIVERY, FALSE) THEN ap.id END) AS total_heavy_or_large
