...
SELECT
    ...
    COUNT(DISTINCT CASE
        WHEN ap.time_slot_start IS NULL
             ...
             AND CASE WHEN ap.payment_method IN ('invoice','delivery_invoice','meal_benefit')
                      ...
                      THEN 'Corporate' ELSE 'Private' END = 'Private'
        THEN ap.id END) AS platform_orders,
    ...
    COUNT(DISTINCT CASE
        WHEN ctg."TIME_TO_LAST_ACCEPT" / 60.0 >= {rotten_threshold_min}
             ...
             AND CASE WHEN ap.payment_method IN ('invoice','delivery_invoice','meal_benefit')
                           OR ap.purchase_type = 'daas'
                      THEN 'Corporate' ELSE 'Private' END = 'Private'
        THEN ap.id END) AS rotten_count
